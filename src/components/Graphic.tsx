...
/*export const valForGraph = () => {
  const StockPrices = [
      ...
      2.266, 2.277, 2.257, 2.248, 2.266, 2.273, 2.257, 2.252, 2.266, 
      2.268, 2.257, 2.277, 2.286, 2.268, 2.261, 2.277, 2.282, 2.268, 
      2.266, 2.277, 2.257, 2.248, 2.266, 2.273, 2.257, 2.252
      ];

  const [currentValues, setCurrentValues] = useState(StockPrices.slice(0, 11));
  const [currentIndex, setCurrentIndex] = useState(11); // Start from the 12th index

  useEffect(() => {
      const intervalId = setInterval(() => {
          // Update currentValues by removing the first item and adding the next StockPrice
...
const StockPriceComponent = () => {
  const currentPrices = valForGraph();
